...
def benchmark_all_eval(model, criterion, converter, opt, calculate_infer_time=False):
    ...
    eval_data_list = ['ic03_867', 'svt_p_645']
    ...
    for i, (image_tensors, labels, _, _) in enumerate(evaluation_loader):
        ...
        if 'CTC' in opt.Prediction:
            ...
            if opt.baiduCTC:
                ...
                _, preds_index = preds.max(2)
            preds_str,_ = converter.decode(preds_index.data, preds_size.data)
            ...
            # select max probabilty (greedy decoding) then decode index to character
            _, preds_index = preds.max(2)
            preds_str,_ = converter.decode(preds_index, length_for_pred)
            labels,_ = converter.decode(text_for_loss[:, 1:], length_for_loss)
...
def test(opt):
    ...
    with torch.no_grad():
        ...
        current_ece, acc, _, pzhECE, flag_str = ACE(preds_data, bin_num=15, vis=True, correct_fn=1, testing=True)
        ece, mce = ECE(preds_data, bin_num=15, correct_fn=None)
        print(ece)
