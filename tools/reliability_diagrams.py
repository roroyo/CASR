...
def _reliability_diagram_subplot(ax, bin_data, draw_order=True,
    ...
    if draw_order == True:
        ...
        acc_plt = ax.bar(positions, accuracies, bottom=0, width=widths,
                         color=ece_styles["acc_color"], edgecolor="black", alpha=1.0, linewidth=5,
                         label="Accuracy")

        gap_plt = ax.bar(positions, np.abs(accuracies - confidences),
                         bottom=np.minimum(accuracies, confidences), width=widths,
                         color=ece_styles["gap_color"], edgecolor="red", alpha=0.6, linewidth=5, label="Gap")
    else:
        acc_plt = ax.bar(positions, accuracies, bottom=0, width=widths,
    ...
    ax.set_aspect("equal")
    ax.plot([0, 1], [0, 1], linestyle="--", color="black", linewidth=8)
...
def _reliability_diagram_combined(bin_data,
   ...
   # plt.grid(color='r', linestyle='--', linewidth=0.5)
    ...
    _confidence_histogram_subplot(ax, bin_data, draw_averages, title="")
...
def reliability_diagram(true_labels, pred_labels, confidences, num_bins=10,
                        ...
                        figsize=(20, 20), dpi=600, return_fig=False):
...
if __name__ == '__main__':
    ...
    with open('debug/easy.json') as f:
        pred = json.load(f)

    ece_styles = {"figsize": (20, 20),  
                 "dpi": 600,           
                 "acc_color": "#0000FF",
                 "gap_color": "#F8BABA"}
    ...
    reliability_diagram(true_labels=gt, pred_labels=preds, confidences=confidence, num_bins=50, figsize=ece_styles['figsize'], dpi=ece_styles['dpi'])
